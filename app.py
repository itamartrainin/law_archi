...
with c1:
    ...
    chart = alt.Chart(vc).mark_bar().encode(
        x=alt.X('speaker:N', sort=None, axis=alt.Axis(labelAngle=45, labelPadding=60)),
        y=alt.Y('count:Q')
    )
...
with c2:
    ...
    # Altair bar chart
    chart = alt.Chart(vc).mark_bar().encode(
        x=alt.X('position:N', sort=None, axis=alt.Axis(labelAngle=45, labelPadding=60)),
        y=alt.Y('count:Q')
    )
